#!/usr/bin/env bash
#
# File de modération — ce que l'agent a réellement sous les yeux.
#
# C'est l'écran principal de l'agent, et personne n'éprouvait son CONTENU :
# une promo signalée y entre-t-elle, une promo résolue en sort-elle ? Une file
# qui rendrait toujours la même liste passait les deux bancs qui la touchaient.
#
# ⚠️ Il crée SA promo et la signale : il ne touche jamais à celle du décor,
# qu'il masquerait pour tous les autres bancs et pour vos tests manuels.
#
# Le détail, les verdicts et l'auto-test sont dans `lib/file_moderation.py`.
#
...
SORTIE_AUTOTEST="$("$PY" "$HERE/lib/file_moderation.py" --self-test)" || {
...
echo
exec "$PY" "$HERE/lib/file_moderation.py" "$@"
