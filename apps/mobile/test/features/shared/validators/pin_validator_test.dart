...
void main() {
  ...
  testWidgets('validatePin — 6 à 12 chiffres (décision produit 2026-07-13)',
      (tester) async {
    ...
    expect(validate('1234567890123'), isNotNull,
        reason: 'rejette plus de 12 chiffres');
    expect(validate('12a456'), isNotNull,
        reason: 'rejette les caractères non numériques');
    expect(validate(null), isNotNull, reason: 'rejette null');
    expect(validate(''), isNotNull, reason: 'rejette vide');
  });

  testWidgets(
      'validateExistingPin — reste permissif sur 4 à 12 chiffres (PIN fixés avant le 2026-07-13)',
      (tester) async {
    final context = await pumpLocalizedContext(tester);
    final validate = validateExistingPin(context);

    expect(validate('1234'), isNull,
        reason: 'accepte un ancien PIN à 4 chiffres');
    expect(validate('123456789012'), isNull, reason: 'accepte 12 chiffres');
