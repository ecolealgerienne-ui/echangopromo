...
hassi-bahbah|+213555000201|35.0774|3.0281|Épicerie·Hassi·Bahbah|Route·de·Djelfa,·Hassi·Bahbah
alger|+213555000301|36.7538|3.0588|Supérette·Alger·Centre|Rue·Didouche·Mourad,·Alger
"
...
for ligne in $VILLES; do
  ...
  # Les `·` remplacent les espaces dans la table : une boucle `for` se coupe sur
  # les espaces, et bricoler un `IFS` pour trois libellés coûterait plus cher à
  # lire que cette substitution, faite ici une fois.
  ...
  [ -n "$cid" ] || {
    echo "$sortie" | tail -12
    echo "❌ $nom : identifiant du commerçant illisible dans la sortie du décor."
    exit 2; }
  ...
  # choses différentes.
  pid="$(curl -s "$API_URL/promo/map?north=$(awk -v v=$lat 'BEGIN{print v+0.05}')&south=$(awk -v v=$lat 'BEGIN{print v-0.05}')&east=$(awk -v v=$lng 'BEGIN{print v+0.05}')&west=$(awk -v v=$lng 'BEGIN{print v-0.05}')"     -H "X-Device-Id: provision-villes"     | CID="$cid" python3 -c "import sys,json,os
try:
...
for c in d.get('items', []):
  ...
  RESUME="$RESUME
export VILLE_${cle}_TEL='$tel'   VILLE_${cle}_LAT='$lat'   VILLE_${cle}_LNG='$lng'
export VILLE_${cle}_CID='$cid'   VILLE_${cle}_PID='$pid'"
done
